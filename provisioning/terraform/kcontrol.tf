...
resource "libvirt_volume" "centos9_image_kcontrol" {
  count = var.kcontrol_machine_count
  name   = "CentOS-Stream-9-x86_64-${var.domain_name}_kcontrol-${count.index}.qcow2"
  pool   = "default"
...
resource "libvirt_domain" "centos9_kcontrol" {
  count = var.kcontrol_machine_count

  name   = format("kcontrol%02d.%s", count.index + 1, var.domain_name)
  memory = var.kcontrol_memory_size
  vcpu   = var.kcontrol_cpu_count
  ...
  disk {
    volume_id = libvirt_volume.centos9_image_kcontrol[count.index].id
  }

  disk {
    volume_id = libvirt_volume.cloud_init_iso_kcontrol[count.index].id
  }
  ...
  depends_on = [
    local_file.user_data_kcontrol,
    local_file.meta_data_kcontrol
  ]
...
resource "local_file" "user_data_kcontrol" {
  count    = var.kcontrol_machine_count
  content  = data.template_file.user_data_kcontrol[count.index].rendered
  filename = format("/tmp/${var.domain_name}_kcontrol%02d/user-data", count.index + 1)
}

data "template_file" "user_data_kcontrol" {
  count    = var.kcontrol_machine_count
  template = <<EOF

#cloud-config
hostname: "kcontrol${count.index}.${var.domain_name}"
users:
  - default
...
### Cleanup network configuration
  ...
  - nmcli connection modify ens3 ipv4.addresses ${var.kcontrol_ips[count.index]}
  - nmcli connection modify ens3 ipv4.gateway ${var.kcontrol_gateway}
  - nmcli connection modify ens3 ipv4.dns ${var.kcontrol_dns}
...
### Update and rename host
  ...
  - dnf update -y
  - hostnamectl set-hostname "kcontrol${count.index + 1}.${var.domain_name}"
...
resource "local_file" "meta_data_kcontrol" {
  count    = var.kcontrol_machine_count
  content  = data.template_file.meta_data_kcontrol[count.index].rendered
  filename = format("/tmp/${var.domain_name}_kcontrol%02d/meta-data", count.index + 1)
}

data "template_file" "meta_data_kcontrol" {
  count    = var.kcontrol_machine_count
  template = <<EOF
instance-id: kcontrol${count.index + 1}
local-hostname: kcontrol${count.index + 1}
EOF
}
...
resource "null_resource" "create_cloud_init_iso_kcontrol" {
  count = var.kcontrol_machine_count

  provisioner "local-exec" {
    command = format("genisoimage -output /tmp/${var.domain_name}_cloud-init-kcontrol-%02d.iso -volid cidata -joliet -rock /tmp/${var.domain_name}_kcontrol%02d/user-data /tmp/${var.domain_name}_kcontrol%02d/meta-data", count.index + 1, count.index + 1, count.index + 1)
  }
...
resource "libvirt_volume" "cloud_init_iso_kcontrol" {
  count   = var.kcontrol_machine_count
  name    = format("${var.domain_name}_cloud-init-kcontrol-%02d.iso", count.index + 1)
  pool    = "default"
  source  = format("/tmp/${var.domain_name}_cloud-init-kcontrol-%02d.iso", count.index + 1)
  format  = "raw"

  depends_on = [
    null_resource.create_cloud_init_iso_kcontrol
  ]
}
