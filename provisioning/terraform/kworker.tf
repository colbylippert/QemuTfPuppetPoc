...
resource "libvirt_volume" "centos9_image_kworker" {
  count = var.kworker_machine_count
  name   = "CentOS-Stream-9-x86_64-${var.domain_name}_kworker-${count.index}.qcow2"
  pool   = "default"
...
resource "libvirt_domain" "centos9_kworker" {
  count = var.kworker_machine_count

  name   = format("kworker%02d.%s", count.index + 1, var.domain_name)
  memory = var.kworker_memory_size
  vcpu   = var.kworker_cpu_count
  ...
  disk {
    volume_id = libvirt_volume.centos9_image_kworker[count.index].id
  }

  disk {
    volume_id = libvirt_volume.cloud_init_iso_kworker[count.index].id
  }
  ...
  depends_on = [
    local_file.user_data_kworker,
    local_file.meta_data_kworker
  ]
...
resource "local_file" "user_data_kworker" {
  count    = var.kworker_machine_count
  content  = data.template_file.user_data_kworker[count.index].rendered
  filename = format("/tmp/${var.domain_name}_kworker%02d/user-data", count.index + 1)
}

data "template_file" "user_data_kworker" {
  count    = var.kworker_machine_count
  template = <<EOF
#cloud-config
hostname: "kworker${count.index}.${var.domain_name}"
users:
  - default
...
### Cleanup network configuration
  ...
  - nmcli connection modify ens3 ipv4.addresses ${var.kworker_ips[count.index]}
  - nmcli connection modify ens3 ipv4.gateway ${var.kworker_gateway}
  - nmcli connection modify ens3 ipv4.dns ${var.kworker_dns}
...
### Update and rename host
  ...
  - dnf update -y
  - hostnamectl set-hostname "kworker${count.index + 1}.${var.domain_name}"
...
resource "local_file" "meta_data_kworker" {
  count    = var.kworker_machine_count
  content  = data.template_file.meta_data_kworker[count.index].rendered
  filename = format("/tmp/${var.domain_name}_kworker%02d/meta-data", count.index + 1)
}

data "template_file" "meta_data_kworker" {
  count    = var.kworker_machine_count
  template = <<EOF
instance-id: kworker${count.index + 1}
local-hostname: kworker${count.index + 1}
EOF
}
...
resource "null_resource" "create_cloud_init_iso_kworker" {
  count = var.kworker_machine_count

  provisioner "local-exec" {
    command = format("genisoimage -output /tmp/${var.domain_name}_cloud-init-kworker-%02d.iso -volid cidata -joliet -rock /tmp/${var.domain_name}_kworker%02d/user-data /tmp/${var.domain_name}_kworker%02d/meta-data", count.index + 1, count.index + 1, count.index + 1)
  }
...
resource "libvirt_volume" "cloud_init_iso_kworker" {
  count   = var.kworker_machine_count
  name    = format("${var.domain_name}_cloud-init-kworker-%02d.iso", count.index + 1)
  pool    = "default"
  source  = format("/tmp/${var.domain_name}_cloud-init-kworker-%02d.iso", count.index + 1)
  format  = "raw"

  depends_on = [
    null_resource.create_cloud_init_iso_kworker
  ]
}
