...
resource "libvirt_volume" "centos9_image_openvpn" {
  name   = "CentOS-Stream-9-x86_64-${var.domain_name}_openvpn.qcow2"
  pool   = "default"
...
resource "libvirt_domain" "centos9_openvpn" {
  count = var.openvpn_deploy_machine ? 1 : 0

  name   = "openvpn.${var.domain_name}"
  memory = var.openvpn_memory_size
  vcpu   = var.openvpn_cpu_count
  ...
  disk {
    volume_id = libvirt_volume.centos9_image_openvpn.id
  }

  disk {
    volume_id = libvirt_volume.cloud_init_iso_openvpn.id
  }

  network_interface {
    bridge = var.public_network
  }
  ...
  depends_on = [
    local_file.user_data_openvpn,
    local_file.meta_data_openvpn
  ]
...
resource "local_file" "user_data_openvpn" {
  content  = data.template_file.user_data_openvpn.rendered
  filename = "/tmp/${var.domain_name}_openvpn/user-data"
}

data "template_file" "user_data_openvpn" {
  template = <<EOF
#cloud-config
hostname: openvpn.${var.domain_name}
users:
  - default
...
### Cleanup network configuration
  ...
  - nmcli connection modify id "Wired connection 2" connection.id ens3
  - nmcli connection modify ens3 ipv4.addresses ${var.openvpn_ip}
  - nmcli connection modify ens3 ipv4.gateway ${var.openvpn_gateway}
  - nmcli connection modify ens3 ipv4.dns ${var.openvpn_dns}
  ...
  - nmcli connection modify id "Wired connection 1" connection.id ens4
  - nmcli connection modify ens4 ipv4.addresses ${var.openvpn_private_ip}
...
### Update and rename host
  ...
  - dnf update -y
  - hostnamectl set-hostname openvpn.${var.domain_name}
...
resource "local_file" "meta_data_openvpn" {
  content  = data.template_file.meta_data_openvpn.rendered
  filename = "/tmp/${var.domain_name}_openvpn/meta-data"
}

data "template_file" "meta_data_openvpn" {
  template = <<EOF
instance-id: openvpn
local-hostname: openvpn
EOF
}
...
resource "null_resource" "create_cloud_init_iso_openvpn" {
  provisioner "local-exec" {
    command = <<EOF
      genisoimage -output /tmp/${var.domain_name}_cloud-init-openvpn.iso -volid cidata -joliet -rock /tmp/${var.domain_name}_openvpn/user-data /tmp/${var.domain_name}_openvpn/meta-data
EOF
  }
...
resource "libvirt_volume" "cloud_init_iso_openvpn" {
  name   = "${var.domain_name}_cloud-init-openvpn.iso"
  pool   = "default"
  source = "/tmp/${var.domain_name}_cloud-init-openvpn.iso"
  format = "raw"

  depends_on = [
    null_resource.create_cloud_init_iso_openvpn
  ]
}
